...
def execute_remote(
    ...
    ssh_cmd = (
        "ssh -o StrictHostKeyChecking=no -p {port} {ip_prefix}{ip} '{cmd}'".format(
            port=str(port),
            ip_prefix=ip_prefix,
            ip=ip,
            cmd=cmd,
...
def construct_dgl_server_env_vars(
    ...
    server_env_vars_template = (
        "--env DGL_ROLE={DGL_ROLE} "
        "--env DGL_NUM_SAMPLER={DGL_NUM_SAMPLER} "
        "--env OMP_NUM_THREADS={OMP_NUM_THREADS} "
        "--env DGL_NUM_CLIENT={DGL_NUM_CLIENT} "
        "--env DGL_CONF_PATH={DGL_CONF_PATH} "
        "--env DGL_IP_CONFIG={DGL_IP_CONFIG} "
        "--env DGL_NUM_SERVER={DGL_NUM_SERVER} "
        "--env DGL_GRAPH_FORMAT={DGL_GRAPH_FORMAT} "
    )
...
def construct_dgl_client_env_vars(
    ...
    client_env_vars_template = (
        "--env DGL_DIST_MODE={DGL_DIST_MODE} "
        "--env DGL_ROLE={DGL_ROLE} "
        "--env DGL_NUM_SAMPLER={DGL_NUM_SAMPLER} "
        "--env DGL_NUM_CLIENT={DGL_NUM_CLIENT} "
        "--env DGL_CONF_PATH={DGL_CONF_PATH} "
    ...
    suffix_optional_envvars = ""
    if pythonpath:
        suffix_optional_envvars += f"--env PYTHONPATH={pythonpath} "
...
def wrap_cmd_with_local_envvars(cmd: str, env_vars: str) -> str:
    ...
    docker_bin = "docker exec"
    new_command = cmd.replace(docker_bin, f"{docker_bin} {env_vars}")
...
def submit_jobs(args, udf_command):
    ...
    for i in range(len(hosts) * server_count_per_machine):
        ...
        server_env_vars_cur = f"{server_env_vars} --env DGL_SERVER_ID={i}"
        cmd = wrap_cmd_with_local_envvars(udf_command, server_env_vars_cur)
        # cmd = 'cd ' + str(args.docker_workspace) + '; ' + cmd
        thread_list.append(
...
def main():
    ...
    parser.add_argument("--port", type=int, help="port", default=30050)
    parser.add_argument(
        "--workspace",
        type=str,
    ...
    parser.add_argument(
        "--num_omp_threads", type=int, help="The number of OMP threads per trainer"
    )
    parser.add_argument(
        "--num_samplers",
        type=int,
    ...
    parser.add_argument(
        "--part_config",
        type=str,
        default="/home/2part_data/ogbn-products.json",
