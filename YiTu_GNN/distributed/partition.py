...
from dgl.transform import metis_partition_assignment, partition_graph_with_halo
from dgl.distributed.graph_partition_book import BasicPartitionBook, RangePartitionBook
from dgl.distributed.graph_services import _sample_neighbors
...
def partition_graph(g, graph_name, num_parts, out_path, num_hops=1, part_method="metis", feat_name='features',
                    reshuffle=True, balance_ntypes=None, balance_edges=False, return_mapping=False,
                    num_trainers_per_machine=1):
    '''将feature按维度划分并保存到part_feat.dgl文件中, node_feats.dgl文件中没有节点的feature, 只有train_mask, test_mask, val_mask等值。 
    ...
    * ``balance_edges`` balances the number of edges in each partition.
    ...
    part_metadata = {'graph_name': graph_name,
                     ...
                     'etypes': etypes}
    # 按照维度切分特征
    part_feats_dim = g.ndata[feat_name].shape[-1] // num_parts
	# 获取reshuffle之后的特征
    g.ndata[feat_name] = g.ndata[feat_name][orig_nids]
    for part_id in range(num_parts):
        ...
        if part_id == num_parts -1:
            part_feats[feat_name] = g.ndata[feat_name][:, part_feats_dim * part_id:]
        else:
            part_feats[feat_name] = g.ndata[feat_name][:, part_feats_dim * part_id : part_feats_dim * (part_id + 1)]
        if num_parts > 1:
            for ntype in g.ntypes:
                ntype_id = g.get_ntype_id(ntype)
                # To get the edges in the input graph, we should use original node IDs.
                ...
                for name in g.nodes[ntype].data:
                    if name in [NID, 'inner_node', feat_name]:
                        continue
                    node_feats[ntype + '/' + name] = F.gather_row(g.nodes[ntype].data[name],
                                                                  local_nodes)
                    # part_feats_dim = g.nodes[ntype].data[name].shape[-1] // num_parts
                    # if part_id == num_parts -1:
                    #     part_feats[ntype + '/' +name] = g.nodes[ntype].data[name][:, part_feats_dim * part_id:]
                    # else:
                    #     part_feats[ntype + '/' +name] = g.nodes[ntype].data[name][:, part_feats_dim * part_id : part_feats_dim * (part_id + 1)]
        ...
        part_feat_file = os.path.join(part_dir, "part_feat.dgl")
        part_metadata['part-{}'.format(part_id)] = {
			...
			'part_feats': os.path.relpath(part_feat_file, out_path)}
        ...
        # 特征按维度切分后的一个部分特征
        save_tensors(part_feat_file, part_feats)
